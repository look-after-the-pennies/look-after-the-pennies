...
<template>
  <div class="container flex login">
    ...
    <form @submit.prevent="login()" class="form login centred w100">
      ...
      ><span v-if="error.length != 0" class="error ml2">{{ error }}</span>
      <span v-if="loading" class="loading ml2">Loading...</span>
    </form>
  </div>
</template>
